...
#pragma comment(lib, "winmm.lib") // timeGetTime DCD
...
typedef DWORD NTAPI _RtlQueryProcessHeapInformation( 
    IN OUT PDEBUG_BUFFER Buffer 
);
...
void fRtlQueryProcessHeapInformation() {
	...
	// DCD
	_RtlQueryProcessHeapInformation *api = (_RtlQueryProcessHeapInformation*)GetProcAddress(LoadLibrary(L"ntdll.dll"), "RtlQueryProcessHeapInformation");
	api(buffer);
...
void allTests() {
	...
	// 8 sCreateDesktop_SwitchDesktop();
	// 9 sCreateProcess_ContinueDebugEvent();
	fRtlQueryProcessDebugInformation();
	fRtlQueryProcessHeapInformation();
	// 12 sOutputDebugString_GetLastError();
	// 13 fBlockInput();
	sGetCurrentProcessId_CreateToolhelp32Snapshot();
	...
	psVirtuaAlloc_VirtualProtect();
	sGetTickCount();
	stimeGetTime();
	sGetSystemTime();
	sGetLocalTime();
	sQueryPerformanceCounter();
	// 28 pThreadHideFromDebugger();
	fNtSetDebugFilterState();
	// 30 sCreateProcess_CreateMutex();
	// 31 fDbgBreakPoint();
}
...
SCZ:
	...
	goto SCZ;
#else
	const int skipTests[] = { 8, 9, 12, 13, 20, 21, 28, 30, 31};
	int id;
	const int *skip = (const int*)&skipTests;
	for (id = 1; id <= 31; ++id) {
		if (id != *skip) {
